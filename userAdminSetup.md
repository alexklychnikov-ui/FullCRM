...
| Действие | Как |
|----------|-----|
| Создать менеджера | форма «Создать пользователя»: email, ФИО, пароль ≥8, роли (часто `manager`) |
| Выдать / сменить роль | чекбоксы ролей у пользователя → **Сохранить роли** |
| Отозвать доступ | **Отозвать доступ** (`is_active=false`, сессии сбрасываются) |
| Вернуть доступ | **Восстановить доступ** |

Роли по умолчанию:

| Роль | Назначение |
|------|------------|
| `admin` | всё + `admin.manage` |
| `manager` | CRM + коммуникации + AI/analytics read (без admin) |
| `analyst` | чтение CRM/comms + AI/analytics |

Ограничения: нельзя отозвать доступ у себя; нельзя оставить организацию без активного `admin`.
...
**URL:** `/settings` или `/settings?tab=analytics`
...
1. Войти под admin → `/settings`.  
2. **Модули:** включить Communications, Analytics, AI → Сохранить.  
3. **Аналитика:** выставить пороги (например 7 / 7) → Сохранить.  
4. **Telegram:** BotFather → токен → `.env` → restart API → статус live.  
5. Создать/открыть контакт → указать Telegram Chat ID.  
6. Клиент пишет боту → **Опрос Telegram**.  
7. **OpenAI:** ключ → `.env` (`AI_MOCK=false`) → restart → проверка на сделке и `/analytics`.  
8. Gmail/Calendar оставить stub до отдельной задачи OAuth.  
...
*Версия инструкции соответствует MVP FullCRM с модулями CRM / Communications / Analytics / AI и интеграциями Telegram (live) + Gmail/Calendar (stub).*
